...
class WPMastertoolkit_Ban_Emails {
	...
	public function verify_emails( $sanitized_user_login, $user_email, $errors ) {
		...
		if ( $email_not_accepted ) {
			...
			$message = sprintf( __( '%sError:%s Your email is not accepted.', 'wpmastertoolkit' ), '<strong>', '</strong>' );
    ...
    public function render_submenu() {
        $submenu_assets = include( WPMASTERTOOLKIT_PLUGIN_PATH . 'admin/assets/build/ban-emails.asset.php' );
        wp_enqueue_style( 'WPMastertoolkit_submenu', WPMASTERTOOLKIT_PLUGIN_URL . 'admin/assets/build/ban-emails.css', array(), $submenu_assets['version'], 'all' );
        wp_enqueue_script( 'WPMastertoolkit_submenu', WPMASTERTOOLKIT_PLUGIN_URL . 'admin/assets/build/ban-emails.js', $submenu_assets['dependencies'], $submenu_assets['version'], true );
