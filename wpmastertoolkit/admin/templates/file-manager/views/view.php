	...
	<div class="col-12">
		...
		<p class="break-word">
			...
			if ( ( $is_zip || $is_gzip ) && $filenames !== false ) {
				...
				echo sprintf( esc_html__( 'Files in archive: %s', 'wpmastertoolkit' ), esc_html( $total_files ) ) . '<br>';
				echo sprintf( esc_html__( 'Total size: %s', 'wpmastertoolkit' ), esc_html( $this->fm_get_filesize( $total_uncomp ) ) ) . '<br>';
				echo sprintf( esc_html__( 'Size in archive: %s', 'wpmastertoolkit' ), esc_html( $this->fm_get_filesize( $total_comp ) ) ) . '<br>';
				echo sprintf( esc_html__( 'Compression: %s%%', 'wpmastertoolkit' ), esc_html( round( ( $total_comp / max( $total_uncomp , 1 ) ) * 100 ) ) ) . '<br>';
			}
			// Image info
			if ( $is_image ) {
				$image_size = getimagesize( $file_path );
				echo '<strong>' . __( 'Image size', 'wpmastertoolkit' ) . ':</strong> ' . ( isset( $image_size[0] ) ? $image_size[0] : '0' ) . ' x ' . ( isset( $image_size[1] ) ? $image_size[1] : '0' ) . '<br>';
			}
			// Text info
			if ( $is_text ) {
				...
				echo '<strong>' . __( 'Charset', 'wpmastertoolkit' ) . ':</strong> ' . ( $is_utf8 ? 'utf-8' : '8 bit' ) . '<br>';
			}
			?>
		</p>
		<div class="d-flex align-items-center mb-3">
			...
			<b class="ms-2"><a href="<?php echo $this->fm_enc($file_url) ?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php esc_html_e( 'Open', 'wpmastertoolkit' ); ?></a></b>
			<?php
			// ZIP actions
			if ( ( $is_zip || $is_gzip ) && $filenames !== false ) {
		...
		if ( $is_onlineViewer ) {
			...
			$content = '<pre class="with-hljs"><code class="' . esc_attr( $hljs_class ) . '">' . $this->fm_enc( $content ) . '</code></pre>';
			echo $content;
		}
		?>
	</div>
